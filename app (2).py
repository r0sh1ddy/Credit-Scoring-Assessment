...
# ─────────────────────────────────────────────────────────────────────────────
# DATA  (5 000 rows default — keeps cloud memory / boot time reasonable)
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def generate_dataset(n: int = 5_000, seed: int = 42) -> pd.DataFrame:
    ...
    labels = np.where(np.clip(s, 0, 1) < 0.38, 0,
                      np.where(np.clip(s, 0, 1) < 0.65, 1, 2))
    return pd.DataFrame({
...
# ─────────────────────────────────────────────────────────────────────────────
def linearise(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def rebin(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["Age_Group"] = pd.cut(
        df["Age"], bins=[17, 25, 35, 45, 60, 75],
        labels=["18-25", "26-35", "36-45", "46-60", "61-75"]).astype(str)
    # Detect whether Annual_Income has already been log-transformed
    inc = (np.expm1(df["Annual_Income"].astype(float))
           if df["Annual_Income"].max() < 30
           else df["Annual_Income"].astype(float))
    df["Income_Bracket"] = pd.cut(
        inc, bins=[0, 30_000, 70_000, 120_000, 1e9],
...
def engineer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Util_x_Delay"]         = (df["Credit_Utilization_Ratio"]
                                  * df["Delay_from_due_date"].fillna(0))
    df["Debt_x_Inquiries"]     = (df["Outstanding_Debt"]
                                  * df["Num_Credit_Inquiries"].fillna(0))
    return df


# ─────────────────────────────────────────────────────────────────────────────
# FULL PIPELINE  (cached; _df prefix prevents Streamlit hashing the DataFrame)
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_resource(show_spinner=False)
def run_pipeline(_df: pd.DataFrame, df_key: str, n_trials: int = 15):
    df = linearise(_df)
    df = rebin(df)
    ...
    # Ensure numpy arrays for model fitting
    ...
    base = {
        ...
        "Gradient Boosting": GradientBoostingClassifier(
            n_estimators=100, max_depth=5, learning_rate=0.08, random_state=SEED),
    ...
    for name, mdl in base.items():
        mdl.fit(X_sm_np, y_sm_np)
        p2 = mdl.predict(X_te_np)
        pr = mdl.predict_proba(X_te_np)
        cv = cross_val_score(
            mdl, X_sm_np, y_sm_np,
        ...
        results[name] = dict(
            ...
            preds    = p2,
            proba    = pr,
            accuracy = accuracy_score(y_te, p2),
            f1_macro = f1_score(y_te, p2, average="macro"),
    ...
    voting.fit(X_sm_np, y_sm_np)
    vp  = voting.predict(X_te_np)
    vr  = voting.predict_proba(X_te_np)
    cv_v = cross_val_score(voting, X_sm_np, y_sm_np, cv=cv5,
                           scoring="roc_auc_ovr", n_jobs=-1)
    results["Voting Ensemble"] = dict(
        model    = voting, preds=vp, proba=vr,
        accuracy = accuracy_score(y_te, vp),
        f1_macro = f1_score(y_te, vp, average="macro"),
    ...
    stack = StackingClassifier(
        [(n, base[n]) for n in top4],
        final_estimator=LogisticRegression(
            max_iter=2000, C=0.5, random_state=SEED),
        cv=5, n_jobs=-1)
    stack.fit(X_sm_np, y_sm_np)
    sp  = stack.predict(X_te_np)
    sr  = stack.predict_proba(X_te_np)
    cv_s = cross_val_score(stack, X_sm_np, y_sm_np, cv=cv5,
                           scoring="roc_auc_ovr", n_jobs=-1)
    results["Stacking"] = dict(
        model    = stack, preds=sp, proba=sr,
        accuracy = accuracy_score(y_te, sp),
        f1_macro = f1_score(y_te, sp, average="macro"),
    ...
    return dict(
        ...
        smote_counts= dict(zip(*np.unique(y_sm_np, return_counts=True))),
        n_raw_feats = X_fe.shape[1],
    )
...
def expected_range(annual_income):
    if annual_income < 30_000:  return 400, 560
    if annual_income < 80_000:  return 540, 700
    if annual_income < 150_000: return 650, 780
    return 700, 820
...
def radar_dims(inp):
    return {
        "Payment\nHistory":  max(0, 100
                                 - (inp.get("Num_of_Delayed_Payment", 0) or 0) / 22 * 80
                                 - (inp.get("Delay_from_due_date",    0) or 0) / 62 * 20),
        "Debt\nManagement":  max(0, 100
                                 - (inp.get("Outstanding_Debt", 0) or 0)
        ...
        "Credit\nActivity":  max(0, 100
                                 ...
                                 - max(0, (inp.get("Credit_Utilization_Ratio", 30) or 30) - 30)
                                 / 20 * 50),
        "Savings &\nIncome": min(100,
                                 (inp.get("Amount_invested_monthly", 0) or 0)
                                 / max(inp.get("Monthly_Inhand_Salary", 1) or 1, 1) * 80
                                 + min(20, (inp.get("Annual_Income", 0) or 0) / 150_000 * 20)),
    }
...
def gauge(score, lo, hi, label):
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={"text": (f"<b>{label}</b><br>"
                        f"<span style='font-size:.8em;color:#888'>Expected {lo}–{hi}</span>"),
               "font": {"color": "#e8e8e8"}},
        number={"font": {"color": col, "size": 50}},
        gauge={
            "axis":      {"range": [300, 850], "tickcolor": "#888", "tickfont": {"color": "#888"}},
    ...
    fig.update_layout(paper_bgcolor=CARD_BG, plot_bgcolor=CARD_BG,
                      font={"color": "#e8e8e8"},
                      margin=dict(l=20, r=20, t=80, b=20), height=310)
    return fig
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 100],
                            tickfont=dict(color="#888"), gridcolor=BORDER),
            angularaxis=dict(tickfont=dict(color="#e8e8e8"), gridcolor=BORDER),
            bgcolor=CARD_BG),
        paper_bgcolor=CARD_BG,
        legend=dict(font=dict(color="#e8e8e8"), bgcolor=CARD_BG),
...
def deviation_chart(user_fe, good_p, poor_p):
    KEYS = [
        ("Outstanding_Debt",        True,  "Outstanding Debt"),
    ...
    for col, lower_better, lbl in KEYS:
        ...
        if lower_better:
            pct = -pct
        rows.append({"Feature": lbl, "Deviation": pct,
                     "User": round(u, 2), "Good": round(g, 2)})
    ...
    fig.update_layout(
        title="Deviation from 'Good' Profile",
...
def shap_waterfall(mdl, x_single, feat_names, cls_idx):
    """Return a Plotly bar chart of top-12 SHAP values for the predicted class."""
    try:
        ex  = shap.TreeExplainer(mdl)
        raw = ex.shap_values(x_single)
        # raw is (n_samples, n_features, n_classes) for multi-class tree models
        ...
        fig.add_vline(x=0, line_color="#888", line_width=1)
        fig.update_layout(
            title=f"SHAP — Why '{CLASS_NAMES[cls_idx]}'?",
...
def _gauge_png(score, lo, hi):
    ...
    zones = [(300, 580, "#4a1515"), (580, 670, "#4a3d0a"),
             (670, 740, "#2a3d0a"), (740, 800, "#0a3d1a"), (800, 850, "#0a4d22")]
    for zl, zh, zc in zones:
        t1 = math.pi * (1 - (zl - 300) / 550)
        t2 = math.pi * (1 - (zh - 300) / 550)
    ...
    ax.set_yticks([]); ax.set_xticks([]); ax.set_ylim(0, 1)
    col = "#4ade80" if score >= 740 else "#fbbf24" if score >= 580 else "#ef4444"
    ax.text(math.pi / 2, 0.32, str(score), ha="center", va="center",
            color=col, fontsize=20, fontweight="bold")
    ax.text(math.pi / 2, 0.12, f"Expected: {lo}–{hi}",
            ha="center", color="#aaa", fontsize=7)
    buf = io.BytesIO()
    plt.tight_layout()
    fig.savefig(buf, format="png", dpi=120, bbox_inches="tight", facecolor="#1a1f2e")
    plt.close(fig)
    buf.seek(0)
    return buf
...
def _radar_png(scores):
    ...
    fig, ax = plt.subplots(figsize=(4, 4), subplot_kw={"polar": True},
                           facecolor="#1a1f2e")
    ax.set_facecolor("#1a1f2e")
    ax.plot(angles, vals, color="#3b82f6", lw=2)
    ax.fill(angles, vals, color="#3b82f6", alpha=0.2)
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(cats, color="#e8e8e8", fontsize=7)
    ax.set_ylim(0, 100); ax.set_yticks([25, 50, 75])
    ax.yaxis.set_tick_params(labelsize=6, labelcolor="#888")
...
def build_pdf(inputs, score, lo, hi, label, proba, tips, radar_sc,
              user_fe, good_p, poor_p):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(0, 11,
             f"  Credit Score: {label}   ({score} / 850)   "
             f"Expected Range: {lo}–{hi}",
             ln=True, fill=True, align="C")
    pdf.ln(3)

    # Gauge + Radar images
    gbuf = _gauge_png(score, lo, hi)
    rbuf = _radar_png(radar_sc)
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as gf:
        gf.write(gbuf.read()); gp = gf.name
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as rf:
        rf.write(rbuf.read()); rp = rf.name
    y0 = pdf.get_y()
    pdf.image(gp, x=15, y=y0, w=88)
    pdf.image(rp, x=108, y=y0, w=88)
    pdf.set_y(y0 + 58)
    ...
    for i, (cn, p) in enumerate(zip(CLASS_NAMES, proba)):
        ...
        pdf.set_fill_color(*rc2)
        pdf.cell(40, 6, cn)
        pdf.cell(int(p * 100), 5, "", fill=True)
        pdf.cell(0, 5, f"  {p:.1%}", ln=True)
    pdf.ln(3)
    ...
    for j in range(0, len(items), 2):
        k1, v1 = items[j]
        k2, v2 = items[j + 1] if j + 1 < len(items) else ("", "")
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(30, 30, 30)
    pdf.cell(0, 7, "Personalised Insights", ln=True)
    pdf.set_font("Helvetica", "", 8)
    for tip in tips:
        clean = (tip.replace("⚠️", "[!]")
                    .replace("✅", "[OK]")
    ...
    pdf.set_text_color(160, 160, 160)
    pdf.cell(0, 5,
             "CreditIQ — For analytical purposes only. Not financial advice.",
             align="C")
    out = io.BytesIO()
    pdf.output(out)
    out.seek(0)
...
def sidebar():
    with st.sidebar:
        ...
        st.markdown("---")
        n_trials = st.slider("Optuna trials", 5, 40, 15, 5)
...
def page_overview(df, pipe):
    ...
    c2.metric("Features",     f"{len(pipe['selected'])} selected / {pipe['n_raw_feats']} total")
    ...
    with col2:
        ...
        rows = ([{"Class": f"{LABEL_MAP[k]} before", "Count": raw.get(k, 0),
                  "Phase": "Before"} for k in sorted(raw)]
                + [{"Class": f"{LABEL_MAP.get(int(k), k)} after", "Count": v,
                    "Phase": "After SMOTE"} for k, v in sc.items()])
    ...
    with col3:
        ...
        lb = pd.DataFrame({
            n: {"Acc": f'{v["accuracy"]:.4f}', "AUC": f'{v["roc_auc"]:.4f}',
                "F1":  f'{v["f1_macro"]:.4f}',
                "CV":  f'{v["cv_mean"]:.3f}±{v["cv_std"]:.3f}'}
    ...
    st.markdown(f"""
    <div class="info-box">
    📐 <b>Preprocessing:</b> Log-transform ({", ".join(LOG_FEATS[:3])}…)
    → Age/Income/Delay re-binning → Label encoding → Median imputation → StandardScaler<br>
    🔬 <b>Feature selection:</b> Mutual info → RF importance → RFECV →
    <b>{len(pipe["selected"])} features selected</b><br>
    ⚖️ <b>SMOTE:</b> Balanced training set → {sum(pipe["smote_counts"].values()):,} samples<br>
    🔧 <b>Optuna:</b> {st.session_state.get("n_trials", 15)} trials ·
    best CV AUC {pipe["study"].best_value:.4f}<br>
    🤖 <b>Models:</b> 9 base classifiers + Soft Voting + Stacking (meta: LR) ·
    Best: <b>{bn}</b>
    </div>
    """, unsafe_allow_html=True)
...
def page_eda(df):
    ...
    t1, t2, t3, t4, t5 = st.tabs(
        ["Distributions", "Skewness & Linearisation",
         "Categorical", "Correlation", "Missing Values"])

    with t1:
        num_cols = (df.select_dtypes(include=np.number)
                      .columns.drop("Credit_Score").tolist())
    ...
    with t2:
        ...
        num_cols2 = df.select_dtypes(include=np.number).columns.drop("Credit_Score").tolist()
        skew_df   = df[num_cols2].skew().sort_values(ascending=False)
        log_df    = df[num_cols2].copy()
        for c in LOG_FEATS:
            if c in log_df.columns:
                log_df[c] = np.log1p(log_df[c].clip(lower=0))
        ...
        fig.add_hline(y=-1, line_dash="dot", line_color="orange")
        fig.update_layout(
            barmode="group", title="Skewness Comparison (|>1| = high skew)",
            paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
            font=dict(color="#e8e8e8"), height=380,
            xaxis=dict(gridcolor=BORDER, tickangle=35), yaxis=dict(gridcolor=BORDER))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with t3:
        ...
        if cat_cols:
            ...
            fig2.update_layout(
                barmode="group", paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
                font=dict(color="#e8e8e8"), height=380,
                xaxis=dict(gridcolor=BORDER), yaxis=dict(gridcolor=BORDER))
            st.plotly_chart(fig2, use_container_width=True)
        else:
    ...
    with t4:
        corr = (df.select_dtypes(include=np.number)
                  .drop("Credit_Score", axis=1).corr())
        fig3, ax3 = plt.subplots(figsize=(14, 11), facecolor=DARK_BG)
        ax3.set_facecolor(CARD_BG)
        sns.heatmap(corr, mask=np.triu(np.ones_like(corr, dtype=bool)),
                    annot=True, fmt=".2f", cmap="RdYlGn",
                    linewidths=0.5, vmin=-1, vmax=1, square=True, ax=ax3,
    ...
    with t5:
        ...
        if miss.empty:
            ...
            fig4 = go.Figure(go.Bar(
                ...
                marker_color="#DD8452"))
            fig4.update_layout(
                xaxis_title="Missing (%)", paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
                font=dict(color="#e8e8e8"), height=380,
                xaxis=dict(gridcolor=BORDER), yaxis=dict(gridcolor=BORDER))
            st.plotly_chart(fig4, use_container_width=True)
            st.dataframe(miss.rename("Missing %").round(2).to_frame(),
...
def page_models(pipe):
    ...
    with t1:
        ...
        for metric, color in [("Accuracy", "#4C72B0"), ("F1 Macro", "#55A868"),
            ...
            fig.add_bar(name=metric, x=names,
                        y=lb[metric].astype(float),
                        marker_color=color, opacity=0.85)
        fig.update_layout(
            barmode="group", paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
            font=dict(color="#e8e8e8"), height=400,
            xaxis=dict(gridcolor=BORDER, tickangle=30),
            yaxis=dict(gridcolor=BORDER, range=[0, 1.1]),
            legend=dict(font=dict(color="#e8e8e8")))
        st.plotly_chart(fig, use_container_width=True)

    with t2:
        mc  = st.selectbox("Model", list(res.keys()),
                           index=list(res.keys()).index(bn))
        ...
        st.code(classification_report(pipe["y_te"], res[mc]["preds"],
                                      target_names=CLASS_NAMES))

    with t3:
        st.subheader(f"ROC Curves — Top 5 Models")
        ybin = label_binarize(pipe["y_te"], classes=[0, 1, 2])
        fig3 = go.Figure()
        top5 = (pd.DataFrame({n: {"auc": v["roc_auc"]} for n, v in res.items()})
                  .T.sort_values("auc", ascending=False).head(5).index.tolist())
        ...
        for i, nm in enumerate(top5):
            for ci, cn in enumerate(CLASS_NAMES):
                ...
                fig3.add_scatter(x=fpr, y=tpr,
                                 line=dict(color=cmap[i], width=1.5),
                                 opacity=0.7,
                                 name=f"{nm[:10]}/{cn} ({auc_i:.3f})")
        fig3.add_scatter(x=[0, 1], y=[0, 1],
                         line=dict(dash="dash", color="#555"))
        fig3.update_layout(
            xaxis_title="FPR", yaxis_title="TPR",
            paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
            font=dict(color="#e8e8e8"), height=450,
            xaxis=dict(gridcolor=BORDER), yaxis=dict(gridcolor=BORDER),
    ...
    with t4:
        ...
        for i, (ax, cn) in enumerate(zip(axes4, CLASS_NAMES)):
            ...
            ax.set_title(f"Class: {cn}", color="#e8e8e8")
    ...
    with t5:
        ...
        with c2:
            ...
            fig5 = px.line(tdf, x="number", y="value",
                           title="Optuna Trial History", markers=True)
            fig5.update_layout(
                paper_bgcolor=DARK_BG, plot_bgcolor=CARD_BG,
        ...
        try:
            ...
            imp_df = pd.DataFrame(
                {"Parameter": list(imp.keys()), "Importance": list(imp.values())})
            fig6 = px.bar(imp_df.sort_values("Importance"),
                          x="Importance", y="Parameter", orientation="h",
    ...
    with t6:
        ...
        with c1:
            ...
            for f in sorted(pipe["selected"]):
                st.markdown(f"- `{f}`")
            cv_r = pipe["rfecv"].cv_results_
            ns   = range(pipe["rfecv"].min_features_to_select,
                         pipe["rfecv"].min_features_to_select
                         + len(cv_r["mean_test_score"]))
            fig7 = go.Figure()
            fig7.add_scatter(x=list(ns), y=cv_r["mean_test_score"],
                             mode="lines+markers",
                             line=dict(color="#4C72B0", width=2))
            fig7.add_vline(x=len(pipe["selected"]),
                           line_color="#ef4444", line_dash="dash")
            fig7.update_layout(
                title="RFECV CV Score vs Feature Count",
        ...
        with c2:
            ...
            xgb_mdl = pipe["base"].get("XGBoost (tuned)",
                                       pipe["base"].get("LightGBM"))
            N   = 300
            Xs  = pipe["X_te_r"].values[:N]
            Xdf = pipe["X_te_r"].iloc[:N].reset_index(drop=True)
            ...
            with st.spinner("Computing SHAP…"):
                try:
                    ...
                    # Handle both (n, f, c) ndarray and list-of-arrays formats
                    if isinstance(raw, np.ndarray) and raw.ndim == 3:
                        sv = raw[:, :, ci]
                    elif isinstance(raw, list):
                        sv = raw[ci]
                    ...
                    plt.sca(ax8)
                    shap.summary_plot(sv, Xdf, plot_type="dot",
                                      max_display=14, show=False, color_bar=True)
                    ax8.set_title(f"SHAP — {cls_c}", color="#e8e8e8")
                    ax8.tick_params(colors="#888")
...
def page_predict(pipe):
    ...
    st.markdown(
        "Enter financial details for an instant prediction "
        "with full visual analysis and PDF report.")
    st.markdown("---")
    ...
    with ca:
        ...
        occ = st.selectbox("Occupation",
                           ["Scientist", "Teacher", "Engineer", "Entrepreneur",
                            ...
                            "Writer", "Architect", "Media_Manager"])
    ...
    with cb:
        ...
        ai  = st.number_input("Annual Income ($)",      5_000,  500_000,  60_000, 1_000)
        od  = st.number_input("Outstanding Debt ($)",       0,    5_000,     800,    50)
        emi = st.number_input("Monthly EMI ($)",             0,    5_000,     250,    10)
        inv = st.number_input("Monthly Investment ($)",      0,    5_000,     300,    50)
        bal = st.number_input("Monthly Balance ($)",         0,    2_000,     400,    50)
    with cc:
    ...
    inp = {
        ...
        "Amount_invested_monthly": inv,
        "Monthly_Balance":         bal,
    }

    # ── Preprocess single row (mirrors pipeline order exactly) ────────────
    ...
    for col, le in pipe["le_map"].items():
        if col in raw.columns:
            ...
            raw[col] = le.transform([val if val in le.classes_ else le.classes_[0]])

    input_cols = [c for c in raw.columns if c != "Credit_Score"]
    raw_imp = pd.DataFrame(
        pipe["imputer"].transform(raw[input_cols]),
        columns=input_cols)
    raw_fe = engineer(raw_imp)

    # Align with scaler's feature set (all columns in X_tr_r)
    for c in pipe["X_tr_r"].columns:
        if c not in raw_fe.columns:
            raw_fe[c] = 0.0

    raw_scaled = pd.DataFrame(
        pipe["scaler"].transform(raw_fe[pipe["X_tr_r"].columns]),
        columns=pipe["X_tr_r"].columns)

    # Select only RFECV-chosen features
    sel = pipe["selected"]
    for c in sel:
        if c not in raw_scaled.columns:
            raw_scaled[c] = 0.0
    raw_final = raw_scaled[sel].values
    ...
    badge    = {"Poor": "badge-poor", "Standard": "badge-standard",
                "Good": "badge-good"}[label]

    # ── Row 1: Badge + Gauge + Radar ─────────────────────────────────────
    r1, r2, r3 = st.columns([1, 2, 2])
    with r1:
        st.markdown("### Your Score")
        st.markdown(f'<div class="score-badge {badge}">{label}</div>',
                    unsafe_allow_html=True)
    ...
    with r2:
        ...
        st.plotly_chart(gauge(score, lo, hi, f"{label} · {score}/850"),
                        use_container_width=True)
    with r3:
        st.markdown("### Risk Radar")
        st.plotly_chart(
            radar_chart(radar_dims({**inp, "Credit_Mix": cm,
                                     "Monthly_Inhand_Salary": ai / 12})),
            use_container_width=True)
    ...
    d1, d2 = st.columns(2)
    # Build user_fe dict with original (non-log) values for deviation chart
    raw_fe_dict = raw_fe.iloc[0].to_dict()
    raw_fe_dict.update({
        "Annual_Income":            ai,
        "Outstanding_Debt":         od,
        "Delay_from_due_date":      dfd,
    ...
    with d1:
        ...
        st.plotly_chart(
            deviation_chart(raw_fe_dict, pipe["good_profile"], pipe["poor_profile"]),
            use_container_width=True)
        st.markdown("""<div class="info-box">
        🟢 Green = closer to <b>Good</b> class average &nbsp;|&nbsp;
        🔴 Red = further from <b>Good</b> class average</div>""",
                    unsafe_allow_html=True)
    with d2:
        ...
        if wfig:
            ...
            st.info("SHAP waterfall unavailable for this model.")
    ...
    tips = []
    if dfd > 20:
        tips.append("⚠️ **High payment delays** — paying on or before the due date "
                    "is the single biggest positive action.")
    if od / (ai + 1) > 0.3:
        tips.append("⚠️ **Elevated debt-to-income ratio** — reducing outstanding debt "
                    "will directly improve your score.")
    if cu > 40:
        tips.append("⚠️ **Credit utilisation above 40%** — aim to keep it below 30% "
                    "of your credit limit.")
    if cha < 3:
        tips.append("💡 **Short credit history** — maintaining existing accounts for "
                    "longer builds a stronger track record.")
    if cm == "Bad":
        tips.append("💡 **Poor credit mix** — a diverse combination of credit types "
                    "signals lower risk to lenders.")
    if ndp > 10:
        tips.append("⚠️ **Many delayed payments on record** — set up automatic payments "
                    "to prevent future delays.")
    if nci > 8:
        tips.append("⚠️ **High credit inquiry count** — multiple recent inquiries "
                    "signal credit-seeking behaviour to lenders.")
    if score >= hi:
        tips.append(f"✅ **Above expected range ({lo}–{hi}) for your income bracket** "
                    "— excellent financial discipline!")
    elif score < lo:
        tips.append(f"💡 **Below expected range ({lo}–{hi}) for your income bracket** "
                    "— focus on the items flagged above.")
    if not tips:
        tips.append("✅ Your credit profile looks strong. "
                    "Maintain consistent payment habits to stay here.")

    for tip in tips:
        cls = ("warn-box" if "⚠️" in tip
               else "good-box" if "✅" in tip
               else "info-box")
    ...
    with st.spinner("Building PDF…"):
        pdf_bytes = build_pdf(
            inp, score, lo, hi, label, proba, tips,
...
def main():
    ...
    key  = hashlib.md5(
        str(df.shape).encode() + str(df.iloc[0].values).encode()
    ).hexdigest()
    n_t  = st.session_state.get("n_trials", 15)

    with st.spinner(
        "🔧 Running ML pipeline — SMOTE · RFECV · Optuna · 11 models "
